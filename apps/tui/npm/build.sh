...
echo "Building ltf v$VERSION for all platforms..."
...
for entry in "${platforms[@]}"; do
  ...
  GOOS=$goos GOARCH=$goarch go build -ldflags "$LDFLAGS" -o "$pkg_dir/bin/$bin_name" ./cmd/ltf

  cat > "$pkg_dir/package.json" <<PKGJSON
{
  "name": "@vvg-ltf1/$suffix",
  "version": "$VERSION",
  "description": "ltf binary for $suffix",
...
for entry in "${platforms[@]}"; do
  ...
  echo "  cd npm/ltf-$suffix && npm publish --access public && cd ../.."
done
echo ""
echo "  # Then publish the main package:"
echo "  cd npm/ltf && npm publish --access public && cd ../.."
echo ""
