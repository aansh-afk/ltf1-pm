  ...
  useEffect(() => {
    ...
    for (let i = 0; i < PARTICLE_CFG.COUNT; i++) {
      particles.push({
        ...
        y: Math.random() * rows,
        speed:
          PARTICLE_CFG.SPEED_MIN +
          Math.random() * (PARTICLE_CFG.SPEED_MAX - PARTICLE_CFG.SPEED_MIN),
    ...
    const flyby = {
      nextStart:
        Date.now() +
        FLYBY.INTERVAL_MIN +
        Math.random() * (FLYBY.INTERVAL_MAX - FLYBY.INTERVAL_MIN),
      ...
      if (!reducedMotion) {
        ...
        for (const p of particles) {
          ...
          if (p.x < 0) {
            ...
            p.brightness = Math.floor(
              Math.random() * PARTICLE_CFG.COLORS.length,
            );
          }
        }
      }
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="py-6 border-t border-[#1F1F23] flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            <Link
              to="/"
              className="text-[#9CA3AF] hover:text-[#6366F1] font-['Inter',sans-serif] text-sm font-bold transition-colors duration-200"
            >
              LTF1
            </Link>
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#6B7280] hover:text-[#6366F1] transition-colors duration-200"
              aria-label="GitHub"
            >
