  ...
  if (compact && !isOpen) {
    return (
      <button
        ...
        className="w-full p-16px bg-[var(--theme-background)] border-2 border-[var(--theme-border)] hover:border-[var(--theme-primary)] transition-colors"
      >
    ...
    <div className="bg-[var(--theme-background)] border-2 border-[var(--theme-border)] p-24px">
      <div className="flex items-center justify-between mb-16px">
        <div className="flex items-center gap-12px">
          ...
          <h3 className="text-brutal-lg font-bold uppercase">Smart Task Generator</h3>
        </div>
        {compact && (
          <button
      ...
      <div className="space-y-16px mb-24px">
        <div>
          <label className="text-brutal-sm font-bold uppercase block mb-8px">
          ...
          <textarea
            ...
            className="w-full p-12px bg-[var(--theme-background-secondary)] border-2 border-[var(--theme-border)] text-brutal-sm placeholder-[var(--theme-foreground-tertiary)] focus:border-[var(--theme-primary)] transition-colors resize-none"
            rows={4}
            disabled={loading}
          />
        </div>
        ...
        <div>
          <button
            ...
            className="flex items-center gap-8px text-brutal-sm hover:text-[var(--theme-primary)] transition-colors"
          >
          ...
          {showAdvanced && (
            <div className="mt-12px">
              <label className="text-brutal-xs font-bold uppercase block mb-8px">
                Epic Title (Optional)
              </label>
              <input
                type="text"
                ...
                className="w-full p-8px bg-[var(--theme-background-secondary)] border border-[var(--theme-border)] text-brutal-sm placeholder-[var(--theme-foreground-tertiary)] focus:border-[var(--theme-primary)] transition-colors"
                disabled={loading}
              />
            </div>
          )}
        ...
        <button
          ...
          className="px-16px py-8px bg-[var(--theme-primary)] text-[var(--theme-background)] font-bold uppercase hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
        >
      ...
      {/* Error Display */}
      {error && (
        <div className="mb-16px p-12px bg-[var(--theme-error)]/10 border border-[var(--theme-error)] text-brutal-sm text-[var(--theme-error)]">
          {error}
        </div>
      )}
        ...
        <div className="space-y-16px">
          ...
          <div className="space-y-8px max-h-400px overflow-y-auto">
            {generatedTasks.map((task, index) => (
              <div 
                key={index}
                className={`border-2 p-12px cursor-pointer transition-all ${
                  task.selected 
                ...
                <div className="flex items-start gap-12px">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-8px mb-4px">
                      <span className="text-brutal-lg">{getTypeIcon(task.type)}</span>
                      <h5 className="text-brutal-sm font-bold">{task.title}</h5>
                    </div>
                    
                    <p className="text-brutal-xs text-[var(--theme-foreground-secondary)] mb-8px">
                      {task.description}
                    </p>
                      ...
                      <div className="mt-8px">
                        ...
                        <ul className="mt-4px space-y-2px">
                          {task.acceptanceCriteria.map((criteria, i) => (
                            <li key={i} className="text-brutal-xs text-[var(--theme-foreground-secondary)] ml-16px">
                              • {criteria}
                            </li>
                          ))}
                        </ul>
          ...
          {/* Action Buttons */}
          <div className="flex items-center gap-12px pt-16px border-t border-[var(--theme-border)]">
            <button
              onClick={handleCreateTasks}
              disabled={loading || generatedTasks.filter(t => t.selected).length === 0}
              className="px-16px py-8px bg-[var(--theme-success)] text-[var(--theme-background)] font-bold uppercase hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
            >
            ...
            <button
              ...
              className="px-16px py-8px border-2 border-[var(--theme-border)] hover:border-[var(--theme-error)] hover:text-[var(--theme-error)] transition-colors font-bold uppercase"
            >
              Clear
            </button>
          </div>
      ...
      {/* Feature Badge */}
      <div className="mt-16px pt-16px border-t border-[var(--theme-border)] flex items-center justify-between">
