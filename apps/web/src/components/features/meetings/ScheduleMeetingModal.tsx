...
function formReducer(state: MeetingFormState, action: MeetingFormAction): MeetingFormState {
  switch (action.type) {
    case 'SET_FIELD':
    ...
    case 'REMOVE_AGENDA':
      if (state.agenda.length > 1) {
        return { ...state, agenda: state.agenda.filter((_, i) => i !== action.index) }
      }
      return state
    default:
      return state
...
function MeetingTypeSelector({ selectedType, onSelectType }: MeetingTypeSelectorProps) {
  return (
    <div>
...
function RecurrenceOptionsPanel({ isRecurring, onRecurringChange, recurrenceFreq, onFreqChange, recurrenceInterval, onIntervalChange, recurrenceEndDate, onEndDateChange }: RecurrenceOptionsPanelProps) {
  return (
    <div>
      ...
      {isRecurring && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-[8px] p-[10px] bg-[var(--theme-background-secondary)] border border-[var(--theme-border)]">
          <div>
            <label htmlFor="schedule-meeting-frequency" className="block text-brutal-xs uppercase mb-[4px]">FREQUENCY</label>
            <select
          ...
          <div>
            <label htmlFor="schedule-meeting-interval" className="block text-brutal-xs uppercase mb-[4px]">INTERVAL</label>
            <input
          ...
          <div>
            <label htmlFor="schedule-meeting-end-date" className="block text-brutal-xs uppercase mb-[4px]">END DATE</label>
            <input
...
function AgendaItemsEditor({ agenda, onUpdate, onAdd, onRemove }: AgendaItemsEditorProps) {
  return (
    <div>
...
function MeetingFormFields({
  title, description, duration, startDate, startTime, location, meetingUrl,
  selectedAttendees, attendeeOptions, onFieldChange
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-[8px]">
        <div>
          <label htmlFor="schedule-meeting-title" className="block text-brutal-sm uppercase mb-[4px]">TITLE</label>
          <input
        ...
        <div>
          <label htmlFor="schedule-meeting-duration" className="block text-brutal-sm uppercase mb-[4px]">DURATION (MINUTES)</label>
          <input
            id="schedule-meeting-duration"
            type="number"
            value={duration}
            onChange={(e) => onFieldChange('duration', parseInt(e.target.value) || 30)}
            min={5}
            max={480}
            className="w-full px-[10px] py-[8px] bg-[var(--theme-background-secondary)] border-2 border-[var(--theme-border)]
                     font-mono text-brutal-sm focus:border-primary-brutalist focus:outline-none"
      ...
      <div>
        <label htmlFor="schedule-meeting-description" className="block text-brutal-sm uppercase mb-[4px]">DESCRIPTION</label>
        <textarea
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-[8px]">
        <div>
          <label htmlFor="schedule-meeting-date" className="block text-brutal-sm uppercase mb-[4px]">
        ...
        <div>
          <label htmlFor="schedule-meeting-time" className="block text-brutal-sm uppercase mb-[4px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-[8px]">
        <div>
          <label htmlFor="schedule-meeting-location" className="block text-brutal-sm uppercase mb-[4px]">
        ...
        <div>
          <label htmlFor="schedule-meeting-url" className="block text-brutal-sm uppercase mb-[4px]">
...
export default function ScheduleMeetingModal({
  ...
  const { title, description, meetingType: type, startDate, startTime, duration, location, meetingUrl, selectedAttendees, isRecurring, recurrenceFreq, recurrenceInterval, recurrenceEndDate, agenda, isCreating } = form
    ...
    if (meeting && isOpen) {
      ...
      if (meeting.recurrence) {
        ...
        if (meeting.recurrence.endDate) {
          data.recurrenceEndDate = new Date(meeting.recurrence.endDate).toISOString().split('T')[0]
        }
      }
  ...
  useEffect(() => {
    if (!meeting) {
      ...
      if (selectedType && template) {
        ...
        formDispatch({ type: 'POPULATE', data: { title: selectedType.label, duration: selectedType.duration } })
      }
    }
  ...
  useEffect(() => {
    if (isOpen && !startDate) {
      ...
      formDispatch({ type: 'POPULATE', data: { startDate: tomorrow.toISOString().split('T')[0], startTime: defaultTimes[type] } })
    }
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-[12px]">
        <MeetingTypeSelector
          selectedType={type}
          onSelectType={(val) => formDispatch({ type: 'SET_FIELD', field: 'meetingType', value: val })}
        />
        ...
        <RecurrenceOptionsPanel
          ...
          onRecurringChange={(val) => formDispatch({ type: 'SET_FIELD', field: 'isRecurring', value: val })}
          recurrenceFreq={recurrenceFreq}
          onFreqChange={(val) => formDispatch({ type: 'SET_FIELD', field: 'recurrenceFreq', value: val })}
          recurrenceInterval={recurrenceInterval}
          onIntervalChange={(val) => formDispatch({ type: 'SET_FIELD', field: 'recurrenceInterval', value: val })}
          recurrenceEndDate={recurrenceEndDate}
          onEndDateChange={(val) => formDispatch({ type: 'SET_FIELD', field: 'recurrenceEndDate', value: val })}
        />
