...
type EditTaskAction =
  | {
      type: "UPDATE";
      field: keyof EditTaskState;
      value: EditTaskState[keyof EditTaskState];
    }
...
function TaskDatesEstimate({
  startDate,
  dueDate,
  estimateHours,
  dispatch,
}: TaskDatesEstimateProps) {
  return (
...
export default function EditTaskModal({
    ...
    try {
      await updateTask({
        ...
        estimate: estimateHours
          ? { hours: parseFloat(estimateHours) }
          : undefined,
  ...
  const priorityOptions = [
    ...
    { value: "high", label: "HIGH", className: "text-[var(--theme-warning)]" },
    { value: "medium", label: "MEDIUM", className: "text-[var(--theme-info)]" },
    {
      value: "low",
      label: "LOW",
      className: "text-[var(--theme-foreground)]/60",
    },
  ];
        ...
        <div className="space-y-[4px]">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-[4px] font-mono text-brutal-xs text-[var(--theme-primary)] hover:text-[var(--theme-info)] transition-colors"
            >
              {useSmartAssignment ? (
                <>
