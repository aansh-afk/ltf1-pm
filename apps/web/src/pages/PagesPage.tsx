...
function AITemplateModal({
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true)
    try {
      const content = await generateTemplate({ prompt: prompt.trim(), projectId })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <div className="p-5 space-y-4">
          ...
          {repoInfo && (
            <div className="flex items-center gap-2 px-2.5 py-1.5 border border-[#22C55E]/20 bg-[#22C55E]/5 text-[10px] font-mono text-[#22C55E]/70 tracking-wider">
              ...
              <span>Context: {(repoInfo as any).fullName}{(repoInfo as any).language ? ` (${(repoInfo as any).language})` : ""}</span>
            </div>
          )}
