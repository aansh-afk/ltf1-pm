...
function hasActiveFilters(taskFilters: TaskFiltersType): boolean {
  return !!(taskFilters.search || taskFilters.status.length > 0 || taskFilters.priority.length > 0 ||
...
function TasksFilterInfoBar({ filteredCount, totalCount, dispatch }: {
  filteredCount: number; totalCount: number; dispatch: React.Dispatch<PageAction>
}) {
  return (
    <div className="bg-[var(--theme-background)] border-2 border-[var(--theme-border)] p-3 flex items-center justify-between">
      <div className="font-['IBM_Plex_Mono',monospace] text-sm">
        SHOWING <span className="font-bold text-[var(--theme-primary)]">{filteredCount}</span> OF <span className="font-bold">{totalCount}</span> TASKS
      </div>
      <button
        onClick={() => {
...
function TasksHeaderControls({ taskView, taskFilters, selectedSprintId, isCompactView, allSprints, dispatch }: {
  taskView: TaskViewType; taskFilters: TaskFiltersType; selectedSprintId: string | null
  isCompactView: boolean; allSprints: any[] | undefined; dispatch: React.Dispatch<PageAction>
}) {
  return (
...
function SprintMetricsBar({ sprintProgress }: {
  sprintProgress: { totalTasks: number; completedTasks: number; percentage: number; velocity: number; daysLeft: number; blockedTasks: number; inReview: number }
}) {
  return (
    <div className="bg-[var(--theme-background)] border-2 border-[var(--theme-border)] p-4">
...
function TasksViewRenderer({ taskView, activeSprint, filteredTasks, projectId, workspaceId, dispatch, handleEditTask, handleDeleteTask, handleDuplicateTask }: {
  taskView: TaskViewType; activeSprint: any; filteredTasks: any[]; projectId: string; workspaceId: string
  dispatch: React.Dispatch<PageAction>; handleEditTask: (task: any) => void
  handleDeleteTask: (task: any) => void; handleDuplicateTask: (task: any) => void
}) {
  return (
    <>
  ...
  return (
    <div className={clsx(
      ...
      {hasActiveFilters(taskFilters) && (
        <TasksFilterInfoBar filteredCount={filteredTasks.length} totalCount={tasks?.length || 0} dispatch={dispatch} />
      )}

      <TasksHeaderControls taskView={taskView} taskFilters={taskFilters} selectedSprintId={selectedSprintId}
        isCompactView={isCompactView} allSprints={allSprints} dispatch={dispatch} />

      {sprintProgress && <SprintMetricsBar sprintProgress={sprintProgress} />}

      <TasksViewRenderer taskView={taskView} activeSprint={activeSprint} filteredTasks={filteredTasks}
        projectId={projectId} workspaceId={workspaceId} dispatch={dispatch}
        handleEditTask={handleEditTask} handleDeleteTask={handleDeleteTask} handleDuplicateTask={handleDuplicateTask} />
    </div>
  )
}
...
function TeamOverviewStats({ memberCount, teamTotals }: {
  memberCount: number; teamTotals: { totalTasks: number; completedTasks: number; inProgressTasks: number; hoursTracked: number; avgProductivity: number }
}) {
  return (
...
function WorkloadDistribution({ memberStats, workloadMax, teamTotals }: {
  memberStats: any[]; workloadMax: number; teamTotals: { totalTasks: number; completedTasks: number; inProgressTasks: number; avgProductivity: number }
}) {
  return (
                ...
                <div className="space-y-2">
                  <div className="relative">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="font-['IBM_Plex_Mono',monospace] text-[10px] font-bold">{taskCount} / {workloadMax} TASKS</span>
                    </div>
                    <div className="h-6 bg-[var(--theme-border)] border-2 border-[var(--theme-border)] relative overflow-hidden">
                      ...
                      <div
                        ...
                        style={{ width: `${Math.min(100, (taskCount / workloadMax) * 100)}%` }}
                      >
...
function TeamMembersGrid({ memberStats, taskFilters, dispatch }: {
  memberStats: any[]; taskFilters: TaskFiltersType; dispatch: React.Dispatch<PageAction>
}) {
  return (
...
function TeamQuickActions({ project, memberStats, teamTotals, dispatch }: {
  project: any; memberStats: any[]; teamTotals: { totalTasks: number; completedTasks: number; avgProductivity: number }; dispatch: React.Dispatch<PageAction>
}) {
  return (
    <div className="bg-[var(--theme-background)] border-2 border-[var(--theme-border)]">
...
function TeamTab({ project, tasks, taskFilters, workspaceId, projectId, dispatch }: TeamTabProps) {
  ...
  const workloadMax = Math.max(20, ...memberStats.map((m: any) => m.tasksAssigned))
  ...
  return (
    <div className="space-y-5">
      ...
      <WorkloadDistribution memberStats={memberStats} workloadMax={workloadMax} teamTotals={teamTotals} />
      <TeamMembersGrid memberStats={memberStats} taskFilters={taskFilters} dispatch={dispatch} />
      ...
      <TeamQuickActions project={project} memberStats={memberStats} teamTotals={teamTotals} dispatch={dispatch} />
    </div>
  )
}
