...
|---------|-------------|-------|
| `ltf1 auth login` | Authenticate via browser OAuth | `--token <jwt>` for API token auth |
| `ltf1 auth logout` | Clear stored credentials | |
| `ltf1 auth status` | Show current auth state | `--json` |
...
| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 project list` | List available projects | `--json` |
| `ltf1 project select [key]` | Select active project interactively or by key | `--json` |
| `ltf1 project info` | Show current project details | `--json` |
| `ltf1 project detect` | Auto-detect project from git remote | |

### Task Management

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 task list` | List tasks in current project | `-s <status>`, `-p <priority>`, `--mine`, `--json` |
| `ltf1 task create <title>` | Create new task | `-d <desc>`, `-t <type>`, `-p <priority>`, `-l <labels>`, `-e <estimate>`, `--due-date`, `--assign <userId>` |
| `ltf1 task view <id>` | View task details | `--json` |
| `ltf1 task update <id>` | Update task fields | Same flags as create |
| `ltf1 task done <id>` | Mark task as done | |
| `ltf1 task assign <id>` | Assign task to user | `--user <userId>` |
| `ltf1 task delete <id>` | Delete task | `--force` |
| `ltf1 task comment <id>` | Add comment | `-m <message>` |
| `ltf1 task mine` | Show my assigned tasks | `--json` |

### Sprint Management

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 sprint list` | List sprints | `--json` |
| `ltf1 sprint create <name>` | Create sprint | `--goal`, `--start-date`, `--end-date` |
| `ltf1 sprint status` | Show current sprint status | `--json` |
| `ltf1 sprint add <taskId>` | Add task to current sprint | |
| `ltf1 sprint remove <taskId>` | Remove task from sprint | |
| `ltf1 sprint backlog` | Show backlog tasks | |
| `ltf1 sprint close` | Close current sprint | |

### AI Commands

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 ai suggest` | AI task suggestions | `--from <source>` |
| `ltf1 ai analyze <taskId>` | AI analysis of task | `--json` |
| `ltf1 ai describe <title>` | Generate task description | |

### Git Integration

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 git status` | Show git + task linking status | |
| `ltf1 git hooks` | Install git hooks | `--uninstall` |
| `ltf1 git config` | Configure git integration | |
| `ltf1 git link <taskId>` | Link task to current branch | |
| `ltf1 git sync` | Sync with remote | |

### Time Tracking

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 time start <taskId>` | Start timer | |
| `ltf1 time stop` | Stop active timer | |
| `ltf1 time status` | Show active timer | |
| `ltf1 time log` | Log manual time entry | `--duration`, `--description` |
| `ltf1 time report` | Show time report | `--period <week/month>`, `--json` |

### Daemon

| Command | Description | Flags |
|---------|-------------|-------|
| `ltf1 daemon start` | Start background watcher | `--foreground`, `--verbose` |
| `ltf1 daemon stop` | Stop daemon | `--force` |
| `ltf1 daemon status` | Show daemon PID and state | |
| `ltf1 daemon logs` | View daemon logs | `--follow`, `--lines <n>`, `--clear` |
...
| `ltf1 notifications` | Show notifications |
| `ltf1 config` | CLI configuration |
| `ltf1 completions` | Generate shell completions |
| `ltf1 release notes` | Generate release notes |
| `ltf1 pr create` | Create pull request |
...
ltf1 dashboard    # Explicit TUI launch
ltf1 -d           # Short flag
```
...
1. User config (`ltf1 config set-web-url`)
...
| Command | Description |
|---------|-------------|
| `ltf1 agent triage` | Show triage queue, accept/reject AI suggestions |
| `ltf1 agent suggest` | Agent analyzes current state, suggests next actions |
| `ltf1 agent status` | Show agent activity summary |
| `ltf1 agent create <description>` | Natural language task creation via agent |
...
| `ltf1 skill info <name>` | Show skill details |
