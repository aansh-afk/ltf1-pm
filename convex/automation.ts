...
export const triggerWorkflow = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Create workflow run
    const runId = await ctx.runMutation(api.automation.createWorkflowRun, {
      workflowId: args.workflowId,
  ...
  handler: async (ctx, args) => {
    ...
    for (const workflow of workflows) {
      ...
      if (shouldRunScheduledWorkflow(workflow, now)) {
        ...
        await ctx.runMutation(api.automation.updateWorkflowLastExecuted, {
          workflowId: workflow._id,
          lastExecutedAt: now,
        })
      }
...
export const updateWorkflowLastExecuted = mutation({
  args: {
    workflowId: v.id("workflows"),
...
async function executeWorkflow(
  ...
  try {
    ...
    const sortedActions = workflow.actions.sort((a: any, b: any) => a.order - b.order)
    ...
    for (const action of sortedActions) {
      ...
      try {
        ...
        await ctx.runMutation(api.automation.updateWorkflowRun, {
          runId,
          status: "failed",
          completedAt: Date.now(),
        ...
        await ctx.runMutation(api.automation.incrementWorkflowErrorCount, {
          workflowId: workflow._id,
        })
...
export const incrementWorkflowErrorCount = mutation({
  args: {
    workflowId: v.id("workflows"),
  },
  handler: async (ctx, args) => {
...
async function executeAction(
  ...
  switch (action.type) {
    ...
    case ACTION_TYPES.IF_THEN_ELSE:
      ...
      if (condition) {
        return await executeAction(ctx, action.config.thenAction, context, previousResults)
      } else if (action.config.elseAction) {
        return await executeAction(ctx, action.config.elseAction, context, previousResults)
      }
      break
...
function shouldRunScheduledWorkflow(workflow: any, now: number): boolean {
  ...
  switch (schedule.type) {
    ...
    case "monthly":
      ...
      return now >= monthlyTime.getTime() && lastExecuted < monthlyTime.getTime()
    ...
    actions: [
      {
        ...
        config: {
          ...
          message: "Task '{{task.title}}' is now overdue. Please update its status.",
        },
        order: 1,
      },
    ],
