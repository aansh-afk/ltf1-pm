...
export const gitlabOAuth = httpAction(async (ctx, request) => {
  ...
  if (url.pathname === "/api/gitlab/auth") {
    ...
    await ctx.runMutation(api.integrations.gitlab.mutations.storeOAuthState, { state })
  ...
  if (url.pathname === "/api/gitlab/callback") {
    ...
    // Verify state
    const isValidState = await ctx.runQuery(api.integrations.gitlab.queries.verifyOAuthState, { state })
    if (!isValidState) {
  ...
  if (url.pathname === "/api/gitlab/disconnect") {
    ...
    await ctx.runMutation(api.integrations.gitlab.mutations.disconnectGitLab, { userId })
